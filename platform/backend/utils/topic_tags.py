"""Obsidian topic tag helpers.

The helpers are intentionally storage-agnostic so both DB vault export and
local file-backed analysis can render the same Topic table cell.
"""
...
def extract_openreview_forum_id(*values: object) -> str:
    """Extract an OpenReview forum id from ids, source refs, or links."""
    for value in values:
        text = str(value or "").strip()
        if not text:
            continue
        match = re.search(r"openreview\.net/(?:forum|pdf)\?id=([^&#\s)]+)", text)
        if match:
