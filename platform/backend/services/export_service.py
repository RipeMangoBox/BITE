"""Export service — async DB → obsidian-vault/analysis/ + obsidian-vault/collection/ sync.

Called after pipeline completion to keep Markdown exports in sync with DB.
Also generates obsidian-vault/collection index from DB on demand.
"""
...
async def export_paper_analysis(
    ...
    """Export a single paper's analysis to obsidian-vault/analysis/ Markdown.
...
async def build_collection_index(
    ...
    """Build obsidian-vault/collection/index.jsonl + navigation pages from DB.
...
async def export_obsidian_vault(
    ...
    if root.exists():
        # Clear contents without removing the root directory.
