...
class IngestWorkflow:
    ...
    async def _persist_paper_report(self, paper_id: UUID, report_data: dict):
        ...
        for sec in sections:
            ...
            if stype == "metadata_overview":
                body = self._strip_metadata_topic_row(body)
    ...
    @staticmethod
    def _strip_metadata_topic_row(body: str) -> str:
        """Exporter owns the Topic row so it can use canonical Obsidian tags."""
        lines = []
        for line in (body or "").splitlines():
            if re.match(r"^\|\s*Topic\s*\|", line, flags=re.IGNORECASE):
                continue
            lines.append(line)
        return "\n".join(lines).strip()
