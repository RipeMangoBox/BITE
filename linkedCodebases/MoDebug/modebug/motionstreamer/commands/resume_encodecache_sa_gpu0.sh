...
SA_ROOT=${SA_ROOT:-$EXP_ROOT/motionstreamer/formal_candidates/trace1_full_eval_sa_cfg_sa_ds_review_20260605_gpu1}
BASE_ROOT=${BASE_ROOT:-$EXP_ROOT/motionstreamer/formal_candidates/trace1_full_eval_ca_cfg_ca_ds_review_20260605_gpu0}
GPU=${GPU:-0}
...
RUN_BASELINE_AND_GUARDS=${RUN_BASELINE_AND_GUARDS:-1}
MODE=${1:-dry-run}
LOCK=${LOCK:-/tmp/modebug_motionstreamer_resume_sa_gpu0.lock}
LOG_FILE=${LOG_FILE:-/tmp/modebug_motionstreamer_resume_sa_gpu0_$(date +%Y%m%d_%H%M%S).log}
CURRENT_OUT_DIR=""
CURRENT_LABEL=""
CURRENT_ACTIVE=0

log() {
  printf '[%(%F %T)T] [MS-RESUME-SA-GPU0] %s\n' -1 "$*" | tee -a "$LOG_FILE"
}
...
run_eval() {
  ...
  if completed_manifest_is_valid "$out_dir"; then
    log "SKIP completed family=$family layers=$layers_arg out=$out_dir"
    return 0
  fi

  log "RUN family=$family layers=$layers_arg out=$out_dir mode=$MODE"
  CURRENT_OUT_DIR="$out_dir"
  CURRENT_LABEL="$family:$layers_arg"
  CURRENT_ACTIVE=1
  local -a execute_arg=()
  ...
  write_exit_record "$out_dir" "$status" "$family:$layers_arg"
  if [[ "$status" -eq 0 ]]; then
    log "OK family=$family layers=$layers_arg"
    return 0
  fi
  if [[ "$status" -eq 2 ]]; then
    log "BLOCKED family=$family layers=$layers_arg (exit 2)"
    return 2
  fi
  log "FAIL family=$family layers=$layers_arg (exit $status)"
  return "$status"
}
...
main() {
  ...
  exec 9>"$LOCK"
  if ! flock -n 9; then
    log "ERROR: another MotionStreamer SA resume appears active: $LOCK"
    return 2
  fi

  log "START MotionStreamer SA resume GPU0 layers=${layer_list[*]} mode=$MODE"
  for layer in "${layer_list[@]}"; do
    run_eval sa "$layer" "$SA_ROOT/sa/layer_${layer}" --layers "$layer"
  done

  if [[ "$RUN_BASELINE_AND_GUARDS" == "1" ]]; then
    run_eval baseline none "$BASE_ROOT/baseline"
    run_eval ca none "$BASE_ROOT/ca" || [[ $? -eq 2 ]]
    run_eval cfg_ca none "$BASE_ROOT/cfg_ca" || [[ $? -eq 2 ]]
  fi

  log "DONE MotionStreamer SA resume GPU0"
}
