...
OUT_ROOT="$EXP_ROOT/motionclr/formal_candidates/trace1_full_eval_ca_cfg_ca_${RUN_DATE}_gpu${GPU}"
...
if [[ "${MODEBUG_DS_APPROVED_EXECUTE:-0}" != "1" ]]; then
  log "ERROR: MotionCLR GPU0 formal eval requires MODEBUG_DS_APPROVED_EXECUTE=1"
  exit 2
fi
...
run_one() {
  ...
  log "START MotionCLR GPU0 $family layer $layer"
  ...
  if [[ "$status" -eq 0 ]]; then
    log "OK MotionCLR GPU0 $family layer $layer"
  elif [[ "$status" -eq 2 ]]; then
    log "BLOCKED MotionCLR GPU0 $family layer $layer (exit 2)"
  else
    log "FAIL MotionCLR GPU0 $family layer $layer (exit $status)"
    return "$status"
  fi
}

main() {
  log "=== MotionCLR GPU0: CA (18 layers) + CFG_CA (18 layers) ==="
  ...
  log "=== MotionCLR GPU0 sweep complete ==="
}
