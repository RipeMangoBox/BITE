...
RUN_ID=${RUN_ID:-p1_molingo_l15_mechanism_gpu0_20260611}
...
run_one() {
    ...
    log "RUN $name: mixer=$mixer alpha=$alpha parallel=$parallel_scale orthogonal=$orthogonal_scale norm_ratio=$norm_ratio"
    cd "$REPO"
    set +e
...
main() {
    if [[ "$GPU" != "0" ]]; then
        log "ERROR: GPU0 queue requires GPU=0, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo L15 mechanism queue GPU0 ==="
    log "RUN_ROOT=$RUN_ROOT"
    log "official_eval layer=$LAYER cfg=$CFG_SCALE sample_steps=$SAMPLE_STEPS acc=$ACC repeat=$REPEAT"

    run_one lapcfg_apg_o025_p100 apg_orthogonal 1.0 1.0 0.25 1.0 0.0 8.0
    run_one norm_clamp_r050 norm_clamp 1.0 1.0 0.0 0.5 0.0 8.0

    summarize
    date -Is > "$RUN_ROOT/gpu0_done.marker"
    log "=== GPU0 mechanism queue complete ==="
}
