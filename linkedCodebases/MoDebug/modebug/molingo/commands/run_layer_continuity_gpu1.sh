...
RUN_ID=${RUN_ID:-p1_molingo_layer_continuity_gpu1_20260612}
...
main() {
    if [[ "$GPU" != "1" ]]; then
        log "ERROR: layer continuity queue requires GPU=1, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo layer continuity GPU1 ==="
    log "RUN_ROOT=$RUN_ROOT"
    log "official_eval layers=11,13 plus L10 fixed endpoint cfg=$CFG_SCALE sample_steps=$SAMPLE_STEPS acc=$ACC repeat=$REPEAT seed=$SEED"

    run_one l11_discrepancy_gate_a100 11 discrepancy_gate 1.0
    run_one l11_residual_gate_a080 11 residual_gate 0.8
    run_one l13_discrepancy_gate_a100 13 discrepancy_gate 1.0
    run_one l13_residual_gate_a080 13 residual_gate 0.8
    run_one l10_residual_gate_a080 10 residual_gate 0.8

    summarize
    date -Is > "$RUN_ROOT/gpu1_done.marker"
    log "=== GPU1 layer continuity complete ==="
}
