...
RUN_ID=${RUN_ID:-p1_molingo_l15_discrepancy_validation_gpu0_20260611}
...
run_one() {
    ...
    local -a cmd=(
        ...
        --cfg_residual_discrepancy_threshold "$threshold"
        --cfg_residual_discrepancy_slope "$slope"
    ...
    log "RUN $name: mixer=$mixer alpha=$alpha threshold=$threshold slope=$slope seed=$seed"
    cd "$REPO"
    set +e
...
main() {
    if [[ "$GPU" != "0" ]]; then
        log "ERROR: validation queue requires GPU=0, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo L15 discrepancy validation GPU0 ==="
    log "RUN_ROOT=$RUN_ROOT"

    run_one discrepancy_gate_seed42 discrepancy_gate 1.0 0.0 8.0 42
    run_one discrepancy_gate_seed123 discrepancy_gate 1.0 0.0 8.0 123
    run_one discrepancy_gate_seed456 discrepancy_gate 1.0 0.0 8.0 456
    run_one residual_gate_a080 residual_gate 0.8 0.0 8.0 3407
    run_one residual_gate_a060 residual_gate 0.6 0.0 8.0 3407
    run_one residual_gate_a040 residual_gate 0.4 0.0 8.0 3407

    summarize
    date -Is > "$RUN_ROOT/gpu0_done.marker"
    log "=== GPU0 discrepancy validation complete ==="
}
