...
RUN_ID=${RUN_ID:-p1_molingo_layer_continuity_l11_l10_gpu1_20260612}
...
main() {
    if [[ "$GPU" != "1" ]]; then
        log "ERROR: L11/L10 continuity queue requires GPU=1, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo L11/L10 layer continuity GPU1 ==="
    log "RUN_ROOT=$RUN_ROOT"
    log "official_eval L11 continuity plus L10 fixed endpoint cfg=$CFG_SCALE sample_steps=$SAMPLE_STEPS acc=$ACC repeat=$REPEAT seed=$SEED"
    ...
    log "=== GPU1 L11/L10 continuity complete ==="
}
