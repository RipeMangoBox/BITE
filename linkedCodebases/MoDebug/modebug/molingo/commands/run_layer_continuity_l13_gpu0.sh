...
RUN_ID=${RUN_ID:-p1_molingo_layer_continuity_l13_gpu0_20260612}
...
main() {
    if [[ "$GPU" != "0" ]]; then
        log "ERROR: L13 continuity queue requires GPU=0, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo L13 layer continuity GPU0 ==="
    log "RUN_ROOT=$RUN_ROOT"
    log "official_eval L13 continuity cfg=$CFG_SCALE sample_steps=$SAMPLE_STEPS acc=$ACC repeat=$REPEAT seed=$SEED"
    ...
    log "=== GPU0 L13 continuity complete ==="
}
