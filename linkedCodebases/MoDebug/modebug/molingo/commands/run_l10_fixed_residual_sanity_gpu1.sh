...
RUN_ID=${RUN_ID:-p1_molingo_l10_fixed_residual_sanity_gpu1_20260612}
...
main() {
    if [[ "$GPU" != "1" ]]; then
        log "ERROR: L10 fixed residual sanity requires GPU=1, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo L10 fixed residual sanity GPU1 ==="
    log "RUN_ROOT=$RUN_ROOT"
    log "official_eval layer=10 cfg=$CFG_SCALE sample_steps=$SAMPLE_STEPS acc=$ACC repeat=$REPEAT seed=$SEED"

    run_one l10_residual_gate_a080 10 residual_gate 0.8

    summarize
    date -Is > "$RUN_ROOT/gpu1_done.marker"
    log "=== GPU1 L10 fixed residual sanity complete ==="
}
