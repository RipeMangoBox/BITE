...
RUN_ID=${RUN_ID:-p1_molingo_l10_l15_core_controls_gpu1_20260611}
...
run_one() {
    ...
    log "RUN $name: layer=$layer mixer=$mixer alpha=$alpha threshold=$threshold slope=$slope seed=$SEED"
    cd "$REPO"
    set +e
...
main() {
    if [[ "$GPU" != "1" ]]; then
        log "ERROR: core controls queue requires GPU=1, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo L10/L15 core controls GPU1 ==="
    log "RUN_ROOT=$RUN_ROOT"

    run_one l15_replace_current 15 replace 1.0 0.0 8.0
    run_one l10_discrepancy_gate_t000_s800 10 discrepancy_gate 1.0 0.0 8.0
    run_one l10_replace_current 10 replace 1.0 0.0 8.0

    summarize
    date -Is > "$RUN_ROOT/gpu1_done.marker"
    log "=== GPU1 core controls complete ==="
}
