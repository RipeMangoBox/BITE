...
RUN_ID=${RUN_ID:-p1_molingo_layer_sanity_gpu1_20260612}
...
main() {
    if [[ "$GPU" != "1" ]]; then
        log "ERROR: layer sanity queue requires GPU=1, got GPU=$GPU"
        return 1
    fi

    log "=== MoLingo layer sanity GPU1 ==="
    log "RUN_ROOT=$RUN_ROOT"
    log "official_eval layers=12,14 cfg=$CFG_SCALE sample_steps=$SAMPLE_STEPS acc=$ACC repeat=$REPEAT seed=$SEED"
    log "L16 is intentionally skipped because native MoLingo decoder layers are indexed 0-15."

    run_one l12_discrepancy_gate_a100 12 discrepancy_gate 1.0
    run_one l12_residual_gate_a080 12 residual_gate 0.8
    run_one l14_discrepancy_gate_a100 14 discrepancy_gate 1.0
    run_one l14_residual_gate_a080 14 residual_gate 0.8

    summarize
    date -Is > "$RUN_ROOT/gpu1_done.marker"
    log "=== GPU1 layer sanity complete ==="
}
