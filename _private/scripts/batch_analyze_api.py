...
Environment variables:
  ...
  export GPT_BASE_URL="https://api.vip1129.cc"
...
Usage:
  ...
  cd <repo_root>
  python3 _private/scripts/batch_analyze_api.py [--batch-size 8] [--max-papers 100]
  python3 _private/scripts/batch_analyze_api.py --api-model claude [...]
"""
...
def _dispatch_to_claude(argv: list[str]) -> NoReturn:
    ...
    claude_script = REPO_ROOT / "_private" / "scripts" / "batch_analyze_api_claude.py"
    filtered: list[str] = []
    skip_next = False
    for arg in argv:
        if skip_next:
...
# ── Config ────────────────────────────────────────────────────────
DEFAULT_LOG = "paperAnalysis/processing/github_awesome/showlab__awesome-video-diffusion/analysis_log.csv"
BATCH_SIZE = 1
...
DEFAULT_BASE_URL = "https://api.vip1129.cc"
