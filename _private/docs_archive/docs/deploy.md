...
|----------|---------|-------|
| `POSTGRES_PASSWORD` | `<set-password>`  | Used by `postgres` service AND embedded in `DATABASE_URL`. Keep them in sync. |
| `DATABASE_URL` | `postgresql+asyncpg://rf:<set-password>@postgres:5432/researchflow` | When running outside Docker, swap `postgres` → `localhost`. |
...
| Variable | Example | Notes |
|----------|---------|-------|
| `DEEPSEEK_API_KEY` | _(DeepSeek key)_ | Default analysis-chain provider. |
| `DEEPSEEK_BASE_URL` | `https://api.deepseek.com/v1` | Used when no run-level base URL override is passed. |
| `DEEPSEEK_MODEL` | `deepseek-chat` | Default local analysis model. |
| `KIMI_API_KEY` / `KIMI_BASE_URL` / `KIMI_MODEL` | _(optional)_ | Used only when a run explicitly selects `--provider kimi`. |
| `ANTHROPIC_API_KEY` / `ANTHROPIC_BASE_URL` | _(optional)_ | Reserved for compatible integrations outside the default analysis chain. |
