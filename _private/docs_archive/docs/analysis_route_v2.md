...
重分析应通过通用 ingest workflow 执行，并显式声明目标集合、选择规则和
预算。图片资产可以落库和导出，但默认不会上传给 LLM writer，也不会放进
agent context 的 URL 字段。
...
- 将 MinerU figure asset upload 纳入通用 `parse_service.py` 主路径。
