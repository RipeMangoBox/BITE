...
def test_load_topic_assignments_skips_bad_rows(tmp_path):
    path = tmp_path / "assignments.jsonl"
    path.write_text(
        '{"openreview_forum_id":"abc","root_id":"vision_multimodal_applications"}\n'
        'not-json\n'
        '{"root_id":"missing_id"}\n',
        encoding="utf-8",
    )

    assert _load_topic_assignments(path) == {
        "abc": {
