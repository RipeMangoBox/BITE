...
def test_render_info_table_includes_project_link_from_source_links():
    ...
    assert "[Project](https://example.org/project)" in table
...
def test_extract_source_links_finds_project_page_bare_url():
    links = runner.extract_source_links(
        "Extensive experiments. Project page: https://example.org/project."
    )

    assert links == [{"label": "Project", "url": "https://example.org/project"}]
