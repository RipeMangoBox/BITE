...
if [[ "$GPU" != "1" ]]; then
  echo "ERROR: GPU1 command got GPU=$GPU" >&2
  exit 1
fi

if [[ "$DRY_RUN" != "1" && "${MODEBUG_DS_APPROVED_EXECUTE:-0}" != "1" ]]; then
  echo "ERROR: formal MotionGPT GPU1 eval requires MODEBUG_DS_APPROVED_EXECUTE=1 after DS approval." >&2
  exit 2
fi
...
case "$FAMILY" in
  sa|cfg_sa) ;;
  *)
    echo "ERROR: GPU1 MotionGPT command supports only sa, cfg_sa; got FAMILY=$FAMILY" >&2
    exit 1
    ;;
esac
