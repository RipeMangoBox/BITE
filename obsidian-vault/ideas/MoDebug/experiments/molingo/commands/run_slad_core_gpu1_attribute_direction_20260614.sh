...
RUN_STAMP=${RUN_STAMP:-20260614_core_seed3_mvp_gpu1}

M0_RUN_ID=${M0_RUN_ID:-slad_m0_multiseed_attribute_direction_${RUN_STAMP}}
GDC_RUN_ID=${GDC_RUN_ID:-slad_gdc_probe_attribute_direction_${RUN_STAMP}}
M0_RUN_ROOT="$EXP_ROOT/molingo/slad/$M0_RUN_ID"
GDC_RUN_ROOT="$EXP_ROOT/molingo/slad/$GDC_RUN_ID"
ANALYSIS_ROOT="$EXP_ROOT/molingo/slad/slad_core_calibration_attribute_direction_${RUN_STAMP}"
...
run_m0() {
    mkdir -p "$M0_RUN_ROOT"
    log "=== GPU1 M0 multiseed attribute/direction ==="
...
run_gdc() {
    ...
    log "=== GPU1 GDC probe attribute/direction ==="
...
run_analysis() {
    ...
    log "=== GPU1 CPU calibration summary ==="
...
main() {
    if [[ "$GPU" != "1" ]]; then
        log "ERROR: GPU1 attribute/direction core suite requires GPU=1, got GPU=$GPU"
        return 1
    fi
    ...
    log "=== MoDebug SLAD core GPU1 attribute/direction queue ==="
    ...
    run_gdc
    run_analysis
    log "=== GPU1 attribute/direction core queue finished ==="
}
