...
RUN_ID=${RUN_ID:-slad_m0_suite_action_control_seed3407_official_20260613_gpu0}
...
main() {
    if [[ "$GPU" != "0" ]]; then
        log "ERROR: GPU0 action/control suite requires GPU=0, got GPU=$GPU"
        return 1
    fi
    ...
    log "=== MoDebug SLAD M0 GPU0 action/control suite ==="
    ...
    log "=== GPU0 action/control suite finished ==="
}
