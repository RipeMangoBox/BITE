...
OUT_ROOT="$EXP_ROOT/molingo/formal_candidates/trace1_full_eval_ca_cfg_ca_ds_review_${RUN_DATE}_gpu0"
COMMAND_SCRIPT="$EXP_ROOT/molingo/commands/trace1_full_eval_gpu0_ca_cfg_ca_command.sh"

if [[ "$GPU" != "0" ]]; then
  echo "ERROR: this command is GPU0-only; got GPU=$GPU" >&2
  exit 1
fi
...
"$CONDA_BIN" run --no-capture-output -n "$CONDA_ENV" python "$SCRIPT" \
  ...
  --schema_preflight \
  --families baseline,ca,cfg_ca \
  --expected_layers 18 \
  --gpu_id "$GPU" \
  --conda_bin "$CONDA_BIN" \
  --conda_env "$CONDA_ENV"

echo "MoLingo GPU0 strict baseline+ca+cfg_ca schema preflight passed. No long run is defined in this script."
