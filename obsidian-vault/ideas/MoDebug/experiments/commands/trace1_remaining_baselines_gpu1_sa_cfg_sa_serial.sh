...
declare -A COMMAND=(
  [motiongpt]="$EXP_ROOT/motiongpt/commands/trace1_full_eval_gpu1_sa_command.sh"
  [motionstreamer]="$EXP_ROOT/motionstreamer/commands/trace1_full_eval_gpu1_sa_cfg_sa_command.sh"
  [molingo]="$EXP_ROOT/molingo/commands/trace1_full_eval_gpu1_sa_cfg_sa_command.sh"
)
...
run_baseline_preflight() {
  ...
  if [[ ! -f "$command" ]]; then
    log "BLOCK $baseline GPU$GPU sa+cfg_sa"
    log "Missing baseline-specific command script: $command"
    return 2
  fi

  log "Running $baseline GPU$GPU sa+cfg_sa preflight via $command"
  case "$baseline" in
    motiongpt)
      GPU="$GPU" RUN_DATE="$RUN_DATE" DRY_RUN=1 FAMILY=sa LAYER=0 bash "$command" || return $?
      run_expected_blockable env GPU="$GPU" RUN_DATE="$RUN_DATE" DRY_RUN=1 FAMILY=cfg_sa bash "$command" || return $?
      ;;
    *)
...
main() {
  if [[ "$GPU" != "1" ]]; then
    log "ERROR: this script is GPU1-only; got GPU=$GPU"
    return 1
  fi
  ...
  log "GPU1 preflight summary: blocked=${BLOCKED[*]:-none} failures=${FAILURES[*]:-none}"
