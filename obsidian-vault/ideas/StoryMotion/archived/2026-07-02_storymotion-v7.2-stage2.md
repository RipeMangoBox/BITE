...
updated: 2026-07-03T00:16:54+0800
---
...
## 7. 实测后裁决 2026-07-02 / 2026-07-03

完整指标见 [[2026-07-01_storymotion-v7.2-metric-data|StoryMotion Metric Data]] 的 `StoryMotion v7.2 Official Eval 2026-07-02` 与 `Clean-Preserving CP Follow-up 2026-07-02`。

| item | measured result | decision |
| --- | --- | --- |
| E4 RelationSurrogate | v7.2 系列最佳 camera clean/noise tradeoff：clean FDCLaTr `77.08`、F1 `0.381`；noise FDCLaTr `375.45`、F1 `0.135` | 只作为后续 qualitative / ablation checkpoint，不 promoted |
| E6 camera-safe FT | joint 略好于 E4：joint FDTMR `188.83`、joint F1 `0.283`；但 camera clean 弱于 E4：FDCLaTr `92.24`、F1 `0.368` | 不作为 camera-safe fix |
| E0 clean anchor | camera clean FDCLaTr `15.01`、F1 `0.629`，但 observed-noise FDCLaTr `533.45`、F1 `0.095` | 继续作为 clean H2C 质量锚点 |
| E5 TrustGate probe | gate correct mean 接近 `1.0`，missing/wrong tag 有可测差异但整体饱和 | TrustGate 不是死变量，但不能宣称解决 reliability |
| CP1 low-rel from E0 | clean FDCLaTr `15.75`、F1 `0.646`；noise `0.15` FDCLaTr `547.39`、F1 `0.100` | clean 保住，noise 未改善 |
| CP2 mid-rel from E0 | clean FDCLaTr `15.50`、F1 `0.634`；noise `0.15` FDCLaTr `553.73`、F1 `0.098` | clean 保住，noise 仍 collapse |
| CP3 clean-heavy from E4 | clean FDCLaTr `31.55`、F1 `0.497`；noise `0.15` FDCLaTr `432.89`、F1 `0.120` | 比 E4 clean 好，但没回到 E0/CP1，且 noise 弱于 E4 |

结论：

- E1-E6 与 CP1/CP2/CP3 都已完成核心 official/probe eval，但没有一个可以作为最终方法 promoted。
- 当前最强负结果是 CP1/CP2：它们能保住 E0 clean camera，却完全没有降低 noisy-source collapse。这说明“在 clean anchor 上轻量加入 soft-source / trust / relation”不足以解决核心问题。
- CP3 说明从 E4 往 clean-heavy 回拉只能得到折中点：clean 有改善，但仍弱于 clean anchor，并且丢掉一部分 E4 的 noise 改善。
- v7.2 当前证据只支持“source reliability 是真实瓶颈”，不支持“TextRoleRouter、TrustGate、RelationSurrogate 当前版本已经解决 H2C reliability”。
- 当前 official eval sampler 没有显式传入 v7.2 `task` / `source_meta`，因此不能把 full official 结果强归因给具体 gate 机制。它仍然是 checkpoint 实际表现的有效读数，但不是机制证明。
- 暂停把 v7.2/CP 系列写成主方法；下一步应重新定义 Stage2 conditioning contract，或收缩 StoryMotion 的研究 claim。

## 8. 最终负结论

```text
StoryMotion 的核心问题不是 clean H2C 能力不足，而是 camera branch 在 generated/noisy human source 下没有可靠的 conditioning contract。v7.2 和后续 CP 实验均未解决这个问题：要么保住 clean 但 noise 崩溃，要么略改善 noise 但 clean 大幅退化。
```

因此目前只能说：

- Stage1 / tokenizer 路线已基本排除为主因。
- clean oracle camera completion 不是充分指标。
- Stage2 source reliability / observed branch contract 是必须重做的地方。
- 当前 v7.2 模块是诊断性负结果，不是可汇报为成功的方法。

## 9. Stage2 重构入口

后续不再沿 v7.2/CP 的局部补丁路线继续加 gate 或 preserving loss。Stage2 backbone 重新设计见 [[2026-07-03_storymotion-stage2-backbone-redesign|StoryMotion Stage2 Backbone Redesign]]。

当前建议的主干决策是：

```text
MotionLab-style Motion-Condition-Motion task contract
  + DanceCamera3D-style asymmetric H2C camera completion
  + GenDoP-style camera representation as optional upgrade
```

Pulp / Towards Storytelling Animations 只保留为 framing eval、auxiliary guidance 和后续 joint refinement 参考，不再作为 Stage2 第一版主干。
