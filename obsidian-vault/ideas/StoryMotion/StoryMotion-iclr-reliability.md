...
hypothesis: |
  ...
  C0-LAT是后续唯一operational mainline。C1REL Stage2 endpoint已训练完成；
  C1REL-w/o-Interaction16 matched Stage2已完成formal评测。
tags:
  - StoryMotion
  - reliability
  - submission-closure
...
> artifact搁置NoInt-HREL Stage2，并单独授权C1REL Stage2及
> `C1REL-w/o-Interaction16` Stage1。strict no-I16的广泛退化被作者裁决为Interaction16
> simple-and-effective的正向Stage1 ablation；其matched Stage2现已启动。WORLD与Matched
> Symmetric不因本次授权进入执行。
...
5. representation冻结后，才以同一Stage1、latent target、decoder与同一Pulp camera caption
   训练一个Matched Symmetric Joint Stage2；它允许Camera loss影响Human，用于检验protected
   asymmetric factorization，不能与表示变化混在同一run。
...
- C1REL Stage2：`paperA_c1rel_rawt0_lat_h105k_c105k_seed17_4090g0_20260804`；
  ...
  Direct-C joint指标中observed-H TMR=15.65／FTD=7.93。该run使用Pulp原始camera caption，
  此即最终文本版本。
...
- C1REL-w/o-I16 matched Stage2：
  ...
  65.43（＋47.25），证明Interaction16对Camera生成不可或缺。
- PulpMotion-Repro Stage1：
  ...
  Camera CLaTr=60.51／FCD=17.68，历时1,915s。Stage2待授权启动。
...
| C1REL / `paperA_c1rel_rawt0_lat_h105k_c105k_seed17_4090g0_20260804` | Stage2 representation diagnostic | formal完成 | formal完成 | joint_parallel formal完成 | Human `105K`＋Camera `105K`训练＋三接口formal完成 | Direct-H TMR=17.28与HREL持平；Direct-C CLaTr=61.99≥PulpMotion；Interaction16对Camera生成关键 |
| PulpMotion-Repro / `paperA_pulpmotion_repro162760_stage1_original_seed17_4090g0_r9_20260806` | Stage1 external-system reproduction | 不适用 | 不适用 | 不适用 | r9 Stage1 `413,075`完成；joint reconstruction formal完成（TMR=15.94／CLaTr=60.51，N=4,053） | PulpMotion joint reconstruction baseline建立；Stage2待授权 |
...
| C1REL-w/o-I16 / `paperA_c1rel_nointeraction16_rawt0_lat_h105k_c105k_seed17_4090g1_20260806` | Stage2 matched component ablation | formal完成 | formal完成 | joint_parallel formal完成 | Human `105K`＋Camera `105K`训练＋三接口formal完成 | Direct-H保持（TMR=17.62 vs C1REL 17.28）；Direct-C CLaTr=43.92远低于C1REL 61.99（−18.07），Interaction16对Camera生成有决定性贡献 |
...
| C1REL / `paperA_c1rel_rawt0_lat_h105k_c105k_seed17_4090g0_20260804` | Stage2三模式 | 4,053 |  |  |  |  |  | formal完成；见语义表 |
...
| C1REL-w/o-I16 / `paperA_c1rel_nointeraction16_rawt0_lat_h105k_c105k_seed17_4090g1_20260806` | Stage2三模式 | 4,053 |  |  |  |  |  | formal完成；见语义表 |
| Matched Symmetric Joint / 尚未创建run | Stage2 symmetric joint |  |  |  |  |  |  | 未授权 |

### 2.8 Stage2语义metric表（embedding-based，2026-08-07 formal）
...
| C1REL | Direct-H | 17.28 | 104.33 | — | — | 0.017046 | 0.030448 | 672s |
| C1REL | Direct-C observed-H | 15.65 (joint) | 7.93 (joint) | 61.99 | 18.18 | — | — | 1,090s |
| C1REL | joint_parallel ※ | 17.28 | 104.33 | 64.75 | 23.20 | 0.017046 | 0.030448 | 1,156s |
| C1REL-noI16 | Direct-H | 17.62 | 92.03 | — | — | 0.016846 | 0.030316 | 665s |
| C1REL-noI16 | Direct-C observed-H | 15.58 (joint) | 9.83 (joint) | 43.92 | 65.43 | — | — | 1,178s |
| C1REL-noI16 | joint_parallel ※ | 17.62 | 92.04 | 46.06 | 84.30 | 0.016845 | 0.030320 | 1,152s |

> ※ C1REL第三模式实际执行的是joint_parallel（H、C同时denoising），不是v11 C0-LAT的sequential
> （先生成H再生成C）。evaluate.py当前不支持sequential模式，需修复以统一评测协议。
...
> latent target定义不同（C48语义不同）。上表并置仅为方便查阅，不是同协议对比。

## 3. 投稿闭环矩阵

| 优先级 | 闭环单元 | 当前artifact事实 | 最小剩余动作 | 是否训练 | 关闭后的claim |
| --- | --- | --- | --- | --- | --- |
| closed representation | HREL-w/o-I16 | seed17 fresh `636K`、pure4,053 true-length formal与10,000次paired bootstrap闭合；Stage1 Camera／framing系统性回退 | Stage2搁置；不再支付Stage2长训预算 | 否 | 只支持Stage1 I16 reconstruction贡献；不作generation necessity claim |
| P0 representation | StoryMotion-C1REL | seed17 Stage1 formal已闭合；Stage2已完成fresh Human `105K`＋GT-H LAT Camera `105K`；2026-08-07三接口formal完成：Direct-H TMR=17.28、Direct-C CLaTr=61.99≥PulpMotion 60.51 | 已完成 | 训练＋评测完成 | C1REL表示支持三接口生成；Camera质量不退化于PulpMotion |
| active component | C1REL-w/o-I16 | strict 176D Stage1 formal已闭合；matched Stage2已完成Human `105K`＋Camera `105K`训练及三接口formal：Direct-C CLaTr=43.92 vs C1REL 61.99（−18.07） | 已完成 | 训练＋评测完成 | Interaction16对Stage2 Camera生成不可或缺 |
| P0 method control | Matched Symmetric Joint | 尚未创建；默认parent已冻结为C0-LAT，必须复用其Stage1、latent target、decoder、数据、exposure与同一Pulp camera caption | 先冻结参数／checkpoint／exposure／GPU-hour／inference-cost合同 | 是，仅Stage2；待授权 | protected asymmetric factorization相对symmetric joint denoising的因果价值 |
| P1 external baseline | PulpMotion-Repro-162K | r6在step20,150 CUDA OOM；r7／r8定位并封存未使用raw decode导致的线性live-allocation增长；r9通过16-batch零增长preflight完成`413,075` steps训练；joint reconstruction formal完成（TMR=15.94／CLaTr=60.51） | Stage1 formal已闭合；Stage2待授权 | Stage1 r9完成 | 外部系统边界；不是StoryMotion组件消融 |
...
- C3-25已有Pulp pure4,053行，不应重复训练。现有PulpMotion native行先审计是否满足exact
  162,760 train identity、split、exposure与owning model；当前`PulpMotion-Repro-162K` Stage1已完成，
  Stage2待授权，不能把StoryMotion representation移植进去。
...
- “interaction16对generation必要”——**现已解禁**。C1REL-noI16 matched Stage2 formal已完成，
  Camera CLaTr −18.07／FCD ＋47.25构成决定性证据。
...
2. C1REL Stage2三接口formal已完成（2026-08-07）；Direct-C CLaTr=61.99≥PulpMotion
   60.51，证明C1REL表示支持三接口生成；
3. strict `C1REL-w/o-Interaction16` Stage1＋Stage2 formal已完成（2026-08-07）；Direct-C
   ...
   证据；
4. 以Camera-native adherence与Human-relative framing联合证据裁决HREL／C1REL；
5. 完成获授权representation所需的最小Stage2；Matched Symmetric须另行授权，不把表示变化与
   factorization变化合并；
6. PulpMotion-Repro-162K Stage1 r9已完成（`413,075` steps，joint reconstruction TMR=15.94／CLaTr=60.51）；
   补其Stage2与同协议baseline表；
7. 冻结所有选择后做sealed audit、盲评、失败分层及复现／成本包；
8. H199 evaluator-only审计仅在选择latent-interface优势claim时执行。
...
| --- | --- | --- | --- |
| **表示选择** | HREL (v9)、HREL-noI16、C1REL、C1REL-noI16 | C0-LAT (HREL Stage1)、C0-GEO (HREL Stage1)、C1REL、C1REL-noI16 | HREL vs C1REL comparison（P1） |
| **组件消融** | I16删除（HREL／C1REL两臂） | I16删除 matched Stage2（Direct-C −18 CLaTr，已闭合） | — |
| **方法控制** | — | — | Matched Symmetric Joint（P2，Stage2 only） |
| **外部基线** | PulpMotion-Repro r9 | — | PulpMotion Stage2 |
...
| # | 实验 | Stage | 理由 |
| --- | --- | --- | --- |
| P1-1 | **C1REL vs HREL comparison** | Stage2 train ×2 | C1REL-noI16 component ablation已完成；HREL vs C1REL representation choice须用同一Stage1 owner裁决 |
