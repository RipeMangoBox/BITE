...
## P0-JC-4/5 matched representation and maturity conclusion (2026-07-22)

The corrected matched `30K` single-step audit separates the representation axis from Stage2 maturity. v8.1A is better on most Human distribution and global/root diagnostics at all five timesteps, while C3-25 is substantially better on Camera semantic, coverage, caption, and decoded trajectory diagnostics at nearly every timestep. Joint single-step inherits C3's Camera advantage, but its Human evidence remains mixed. C3-25 is therefore not a universally easier Human representation; its reliable representation gain is Camera-centered.

The independent matched `105K` system comparison reaches the same scoped conclusion. v8.1A is better on most Direct-H metrics, with C3 only slightly higher in HCov. C3 is decisively better on Direct-C distribution/semantic metrics and on joint Camera, coverage, framing, and Camera geometry; joint Human remains split between C3 FTD/HCov and v8.1A TMR/paired geometry. C3-25 remains mainline because StoryMotion is a coupled Human-Camera system and C3 is stronger on the Camera/joint objective, not because it Pareto-dominates v8.1A on Human completion.

Completion-to-joint degradation already exists in teacher-forced single-step denoising, so it cannot be explained only by accumulated free-rollout error. Direct-C receives clean GT Human latent, whereas joint Camera operates with a simultaneously noised/evolving Human branch; free rollout amplifies this conditional-exposure mismatch. The active intervention is therefore a matched Camera replay-refinement or detached generated/noised-H conditioning test from the same C3-105K parent. Stage1 rollback and joint-loss reweighting remain lower priority.

The visual boundary is unchanged: results remain materially closer through `t≤599`, global/root and heading uncertainty becomes important near `t=799`, and `t=999` is a near-pure-noise stress point rather than a promotion gate. Root-aligned MPJPE removes root translation but retains heading, so it is not labeled as local-pose error.
