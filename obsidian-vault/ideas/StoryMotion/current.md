...
> [!info] P0 condition-exposure screen（2026-07-22）
> C3-105K parent、clean C0-110K、`q(H_gt,t)` Tq-110K 与 joint-pred-H Tj-110K 已完成同 evaluator 的 `N=512` 三-profile screen。Tq 相对 matched C0 修复了 joint Human FDTMR/TMR，并改善 joint Camera FDCLaTr/coverage，但 clean completion、joint caption F1 与 Out 仍未恢复 parent；Tj 广泛退化。C3-105K 保持 mainline，Tq 只进入 timestep 分层归因，Tj 停止。数值与 provenance 只见 [[2026-07-18_storymotion-latent-generatability-stage2-diagnostic-ladder#2026-07-22 P0-JC-6 matched `105K→110K` condition-exposure screen]]。
...
| C3-25 P0-JC-6 / C0、Tq、Tj `105K→110K` family | Stage2 matched `5K` + `N=512` screen | clean-H 与 noisy/generated-H exposure 能否解释 completion→joint gap | 三臂 train/contract 与四 checkpoint × 三 profile screen 已闭合；Tq positive-but-mixed，Tj broad regression | parent 保持 mainline；只允许 Tq timestep 分层归因，不授权 formal4053、继续训练或 Tj 扩展 |
...
1. **P0 — 收敛 Stage2 condition-domain remedy。** C3-105K→110K 的 C0/Tq/Tj matched screen 已闭合：C0 证明额外 5K 会让 completion 与 joint 反向漂移；Tq 支持 `q(H_gt,t)` exposure 能修复部分 joint Human/Camera distribution；Tj 当前 Two-Forward 实现停止。下一步只做 Tq 的低/中/高 timestep 分层鲁棒性与 exposure-dose attribution；在 caption F1/Out 未恢复前，不做 formal4053、不延长训练，也不返回 Stage1。
...
- The matched C3-105K→110K condition-exposure screen is complete. Tq (`q(H_gt,t)`) improves the joint Human distribution and part of joint Camera distribution relative to C0, while Tj current-model Two-Forward regresses broadly; neither replaces the C3-105K parent.
- The next P0 action is timestep-stratified Tq attribution, not longer continuation or formal4053. P1 remains automatic-only: choose a reversible physical/TMR union operating point before materializing any clean manifest.
