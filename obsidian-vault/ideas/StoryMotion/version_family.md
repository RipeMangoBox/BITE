...
- **2026-08-10（exact-init C0-LAT reference与source-row diagnostic closure）：** `sm_c0_lat_true_p2_reference_seed17_4090g1_20260810`使用exact legacy C0-LAT Camera EMA、canonical frozen Human teacher及True-P2 data/evaluator/sampler boundary，完成pure4,053 Direct-H、Direct-C、sequential与10,000次P2−C0 paired bootstrap。结果为mixed Pareto：P2改善Human及sequential Camera geometry，却回退Direct-C geometry与两种Camera接口的semantic／framing。source replay进一步确认C0只训练row0，而formal sequential使用exact-zero、训练未覆盖的row1；固定其余变量改为row0后，4,053/4,053样本的Camera latent与decoded output均非零变化，Human exact。故本事件关闭同初始化reference执行缺口，但不关闭source-row-free严格factorization因果解释，也不选择或授权corrective retrain。正式数字、artifact与sealed-audit hash只见[[StoryMotion-valid-metric-ledger#3.21 exact-initialization C0-LAT reference and source-row diagnostic]]。
- **2026-08-11（observed-Human symmetric route-control closure）：** `sm_p2_obstrue_coupled_cont105k_seed17_4090g0_20260810`与`sm_p2_obstrue_detach_cont105k_seed17_5090g3_20260810`均完成`105K` full-state endpoint、预声明gradient audits及pure4,053三接口formal，并与historical True-P2、exact-init C0-LAT完成matched bootstrap。`observed_human=true`大幅修复historical semantic／framing collapse但显著回退Camera geometry；同route下G-on相对G-off改善Direct-C geometry与sequential ADE/FDE，Human geometry CI均跨零，semantic／framing仍mixed。事件关闭route root-cause执行，不建立全局protected-asymmetry superiority；C0 untrained row1与4090／5090 runtime继续作为因果边界。正式数字与hash只见[[StoryMotion-valid-metric-ledger#3.22 observed=true symmetric route controls pure4,053 formal]]。
