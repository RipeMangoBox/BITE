...
- **2026-08-03（v11 C0 seed23 matched repeat闭合）：** `paperA_v11_c0_lat_30to105k_seed23_4090g0_r2_20260803`与`paperA_v11_c0_geo_30to105k_seed23_4090g1_r2_20260803`均从fresh seed23 parent以full-state边界续至Camera `105K`，并完成pure4,053 Direct-H／Direct-C／sequential、decoded geometry／physical及10,000次paired bootstrap审计。Direct-H frozen-owner replay通过；seed内objective对照与objective内跨seed对照共24项Camera geometry 95% CI全部跨零。独立seed缺口关闭，但没有稳健单一objective胜者；C0-LAT／C0-GEO共同mainline不变。正式数值只见[[StoryMotion-valid-metric-ledger#3.14 v11 C0 seed23 105K pure4,053 matched repeat]]。
...
- **2026-08-09（HREL matched Stage2 formal closure）：** `paperA_p1_hrel_matched_lat_h105k_c105k_seed17_4090g1_20260809`完成Direct-H、Direct-C与formal sequential的pure4,053正式评测及sealed artifact/hash审计；实际评测采用`batch=128 / decode_batch=1`，与训练合同声明的`decode_batch=64`不同，差异由独立protocol exception固定并仅约束本次评测比较。与C1REL seed17的exact ordered-ID 10,000次paired geometry bootstrap闭合，结论为mixed Pareto：HREL在Direct-C geometry及sequential ADE/FDE更强，C1REL Camera semantic aggregate更高，但缺少逐样本semantic/framing paired unit，不能声称单边优越。正式数字与hash只见[[StoryMotion-valid-metric-ledger#3.16 HREL matched Stage2 pure4,053 formal]]；本事件不把HREL之外的Pulp Stage2、true-P2或C1REL seed23运行写成已完成结果。
- **2026-08-10（C1REL seed23 raw-T0 Stage2 repeat audit closure）：** `paperA_c1rel_lat_h105k_c105k_seed23_4090g0_20260809`完成Direct-H、Direct-C与sequential的pure4,053 artifact/hash审计。三模式均`is_causal=false`、无`joint_parallel`，并与seed17 reference共享ordered IDs、sampler、batch/decode batch及owning decoder；10,000次cross-seed paired geometry bootstrap的全部CI跨零。审计JSON位于`/data/public/ripemangobox/Motion/StoryMotion/runs/legacy/eval/stage2/paperA_c1rel_lat_h105k_c105k_seed23_4090g0_20260809/seed17_paired_audit_bootstrap_r2.json`，SHA256=`89246b01674a3e8682d74eaa0c38356f47d70e937d2df934e8a6888c2c46b0a5`。由于两seed使用不同Stage2 checkpoint且均绑定raw Pulp Camera-caption `T0` boundary，该事件只关闭repeatability audit，不升级为caption-matched final evidence或promotion；正式数字只见[[StoryMotion-valid-metric-ledger#3.17 C1REL seed23 raw-T0 Stage2 repeat audit]]。
- **2026-08-10（PulpMotion native Stage2 matched available-data closure）：** `paperA_pulpmotion_repro162760_stage2_original_matched210k_seed17_5090g3_r2_20260809`完成fresh `210K`训练、exact `105K` halfway与`210K` final full-state reload audit，并在显式pure-test 4,053 IDs上完成native joint formal。Pulp与StoryMotion的train/eval ID集合exact匹配为`162,760/4,053`；“subset”只表示原始173,912条Pulp split中11,152条缺必要文件，162,760是StoryMotion当前Stage2完整训练cohort，后续称matched available-data cohort。正式结果很弱，但同evaluator／cohort／config的官方checkpoint sanity通过，故裁决为有效negative system-boundary result，不重训、不做CFG选模。Pulp只有native joint path，representation／owning decoder／mode与StoryMotion不同；正式指标与hash只见[[StoryMotion-valid-metric-ledger#3.18 PulpMotion native Stage2 matched available-data cohort]]。
- **2026-08-10（True-P2 matched symmetric formal closure）：** fresh run `paperA_p2_matched_symmetric_joint_fresh_h105k_joint105k_seed17_4090g1_20260809`从canonical Human `105K`／C0-LAT初始化出发，允许Camera loss更新Human，完成`105K` symmetric-joint endpoint及pure4,053 Direct-H、Direct-C、sequential formal；`joint_parallel=false`、`is_causal=false`。结果是mixed Pareto：相对同evaluator/protocol的P1 HREL secondary control，P2的Human与sequential geometry显著改善，但Direct-C Camera geometry、Camera semantic与framing显著回退，sequential semantic／framing也回退。P1 Human初始化与P2不同，因此paired CI只作evaluator-matched secondary evidence，不能称为严格factorization因果效应；后者仍需将与P2 exact初始化匹配的C0-LAT endpoint通过同一evaluator/noise合同。旧`paperA_p2_1_matched_symmetric_joint_h105k_c105k_seed17_4090g0_20260808`永久保留为invalid provenance，不产生metric row。正式数字见[[StoryMotion-valid-metric-ledger#3.19 True-P2 matched symmetric Stage2 pure4,053 formal]]。
