...
- IISRAR
- ARLISR
core_operator: "通过在线多轮 DPO 学习隐式过程奖励模型，并用新旧策略动作概率比生成步骤级奖励，从而把稀疏回合奖励转化为可组合的密集信用分配信号。"
primary_logic: |
  iStar 从旧策略采样正负轨迹对训练隐式 PRM，再用隐式步骤奖励计算步骤级优势，并与基于结果奖励的回合级优势相加后更新策略模型。这个设计避免额外步骤标注和状态重叠假设，使 agentic RL 在长轨迹、多轮交互任务中获得更稳定的信用分配。
claims:
  - "在 WebShop、VisualSokoban 和 SOTOPIA 上，iStar 相比 RLOO、GiGPO、PRIME 等基线取得更高成功率或目标完成率。"
  - "隐式步骤奖励比 token 级过程奖励、直接合并奖励和环境原始步骤奖励更适合多轮 agentic RL 的信用分配。"
  - "该方法可与 RLOO、GRPO、REINFORCE++、DAPO 等强化学习算法组合使用。"
acceptance: accepted
