...
![[assets/figures/papers/paper_list_l916_https_openaccess_thecvf_com_content_CVPR2026_html_Cao_RDF_MIG_A_Robust_D/figures/003_Table_2.jpg]]
*Table 2: Performance of Synthetic Data on Downstream Semantic Segmentation and Change Detection. We compare multiple data-generation methods by training change detection (CD) and semantic segmentation (SS) models on their outputs, using downstream performance to compare the quality of the synthetic data and, in turn, to indirectly assess the relative merits of the generation methods. ”— ” indicates that the method is not applicable to this task. All downstream models are trained using only synthetic data*
