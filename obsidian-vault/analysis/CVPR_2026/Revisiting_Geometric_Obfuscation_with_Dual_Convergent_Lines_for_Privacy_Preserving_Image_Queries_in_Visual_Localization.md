...
![[assets/figures/papers/paper_list_l2059_https_arxiv_org_abs_2604_22310/figures/003_Figure_2.jpg]]
*Figure 2: Illustration of our DCL construction procedure. We begin by extracting keypoints [12, 25] from the query image. Next, lines are generated by connecting each keypoint to one of the two predefined anchor points located at the top and bottom of the central midline. Finally, the positions of the original keypoints are removed. We show in Sec. 4.2 that DCL effectively prevents the geometry-recovery attack [7] by encouraging erroneous recovery of keypoint locations, subsequently successfully thwarting the inversion attack [40]*
...
![[assets/figures/papers/paper_list_l2059_https_arxiv_org_abs_2604_22310/figures/009_Table_3.jpg]]
*Table 3: Localization results on the Cambridge dataset [19]. P.V. stands for Privacy Vulnerability under recent attacks [7, 37, 40], and R.T. denotes real-time performance. Median position error (cm) (↓) and median rotation error (◦) (↓) are reported*

![[assets/figures/papers/paper_list_l2059_https_arxiv_org_abs_2604_22310/figures/011_Table_4.jpg]]
*Table 4: Localization performance on the Aachen Day & Night dataset [47]. Recall (%) at different thresholds is reported*

![[assets/figures/papers/paper_list_l2059_https_arxiv_org_abs_2604_22310/figures/012_Table_5.jpg]]
*Table 5: Evaluation of localization methods on the 7Scenes dataset [49]. We report median position error (cm.) (↓)/ median rotation error (°) (↓)/ recall at 5cm/5° (%) (↑). Results are presented for Structure-Based Methods (SBM) and Privacy-Preserving Methods (PPM)*
...
![[assets/figures/papers/paper_list_l2059_https_arxiv_org_abs_2604_22310/figures/014_Figure_7.jpg]]
*Figure 7: Example of server-side attack, where inlier feature positions are known. (Left) original image (Right) privacy-related contents (e.g. people) are not revealed after the server-side attack*

![[assets/figures/papers/paper_list_l2059_https_arxiv_org_abs_2604_22310/figures/017_Figure_8.jpg]]
*Figure 8: Visualization of the iterative server-side attack on three different scenes. In (a), black points indicate inlier keypoints from the server map, while colored points indicate ground truth (user’s) query keypoints (left) or their estimated locations after running the iterative attack on DCL (right). For each scene, same colored points in (left) and (right) indicate the same keypoint. In (b), the original query image (left) and the image revealed from the estimated points (right) are shown for each scene*
